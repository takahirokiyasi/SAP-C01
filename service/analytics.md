...
### インスタンスの割り当て
中長期的に使用する場合は`マスターノードやコアノードにはリザーブドインスタンス`を利用して、ジョブに割り当てられる`タスクノードにはスポットインスタンス`を使用することが望ましい
...
データ分析基盤に対するデータの取り込み
と言うETLタスク(バッチ処理)をマネージドで提供するサーバーレスなサービス

各種AWSサービスの間を繋ぐ「糊(Glue)」
...
DynamoDB　S3　Redshift RDS EC2上のRDB　オンプレに対応
