...
APIgatewayとかELBとかStep Functionとか割といろんなサービスで使用できる（https://docs.aws.amazon.com/ja_jp/xray/latest/devguide/xray-services.html）
...
マスターアカウントからメンバーアカウントを作成したときに自動作成される
マスターアカウントのIAMユーザーからメンバーアカウントの管理者アクセスを許可するロールで
メンバーアカウントへのswitchロール時に使う。

### メンバーアカウント
マスターアカウント以外のアカウント

#### ルートユーザー
新しくアカウントを作成すると作成されるユーザー
基本的にはアクセス許可を制限したロールや他のユーザーを作成する時のみに使用する。
MFA設定推奨
...
※`要するにSCPでいくら許可しても実際の権限は与えられない、あくまで範囲設定`拒否はちゃんと拒否される。
...
### 暗黙的拒否
FullAWSAccessを付けずに、ホワイトリスト方式でやって、権限付けたいやつだけ許可した場合
明示的に権限つけたやつ以外のものは暗黙的拒否になって使えなくなる

## 請求
メンバーアカウントからは自アカウント分の請求情報のみ参照可能
### 統合請求（一括請求）
...
#### コスト配分タグ
AWS Billing and Cost Managementの機能
複数アカウントに分けても共通のタグ運用をすることによってアカウントを跨る柔軟なコスト管理が可能
例）プロジェクトごとにアカウントを分けて、本番環境と開発環境でコスト配分するとか、チームごとでコスト配分タグつけるとか

### リザーブドインスタンス
アカウントAが購入した同じインスタンスタイプで同じAZが指定されたリザーブドインスタンスを、アカウントBが起動すると、統合請求による価格割引の恩恵を受けることができる（アカウントAでインスタンスの使用なしの時）

## 組織の証跡（CloudTrailとの統合）
AWS Organizationsを利用してマスターアカウントのCloudTrailにおいて組織の証跡を有効化することで、全てのメンバーアカウントでのログ取得を可能にすることができる。各AWSアカウントに関連するログをプライマリアカウントの単一のS3バケットに配信する設定も可能

AWS組織の証跡は、組織内のすべてのメンバーアカウントに自動的に適用。
メンバーアカウントは組織の証跡を表示できますが、これを`変更または削除することはできない`。
デフォルトでは、メンバーアカウントは Amazon S3 バケット内にある組織の証跡のログファイルにはアクセスできない
